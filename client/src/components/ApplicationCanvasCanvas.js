...
function CanvasState(canvas) {
  ...
  canvas.addEventListener('mousedown', function(e) {
    ...
    if (tmpSelected === false) {
      ...
      myState.selection.startX = mx,
      myState.selection.startY = my,
      myState.selection.skewing = e.shiftKey;
  ...
  mouseMoveSelected = function(e, shape) {
    ...
    if ( shape.skewing )
    {
      shape.skewY = mouseX - shape.startX;
      shape.skewX = mouseY - shape.startY;
    }
    else
    if (myState.dragTL) {
      e.target.style.cursor='nw-resize';
