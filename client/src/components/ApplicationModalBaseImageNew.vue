	...
	<TheModal :show="show" @close="close" :title='title'>
		<div slot='body'>
			<div class='flex justify-between'>
				<Button icon='file-image' @click='onClickUpload'>Select Image</Button>
				<Button :class='shouldDisableUpload ? "disabled" : ""' color='green' icon='upload' @click='onClickSave'>Upload</Button>
			</div>

			<div v-for='preview in filePreviews' class='flex bg-white dark:bg-gray-700 dark:border-gray-800 border rounded-lg shadow-lg my-6 w-full'>
...
export default {
	...
	methods: {
		...
		async onClickSave() {
			...
			await this.$store.dispatch('User/Application/addBaseImage', formData);
		}
	}
};
</script>
